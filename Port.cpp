...
void MainWindow::ReadData()
{
    ...
    if (!buf.isEmpty())//判断缓冲区的返回值
    {
        ...
        QString redata = QString("received:%1").arg(QString::number(receBytes));//读取数据
//        ui->ReceiveBrowser->setText(redata);//显示在setText窗口
        ...
        ui->ReceiveBrowser->append(str);//显示数据
...
{
    if (ui->openButton->text() == tr("打开串口"))//串口按钮文字变化
    {
        ...
        if(serialPort->open(QIODevice::ReadWrite))
        {
            ...
            ui->openButton->setText(tr("关闭串口"));//串口按钮文字变化
